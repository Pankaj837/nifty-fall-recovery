...
\- \*\*Backtest (demonstration only, `08\_backtest.ipynb`):\*\* 131 trades, net return -22.7% (gross -11.9%), max trade-level drawdown -52.7%, time in market 14%. Buy and hold returned +336.8%.
...
\- Bootstrap CIs vary slightly between runs and settings (seed and n\_boot).
...
Run the notebooks in order: `01\_data` (download, or use the saved `data/raw\_nifty.csv`), `02\_validation`, `03\_events`, `04\_stats`, `05\_robustness`, `06\_oos` (run once), `07\_falsification`, `08\_backtest`.

The engine is `src/engine.py` (configure with `Config(threshold, hold, decluster, cost\_per\_side)`) and the statistics are in `src/research\_stats.py`.
